...
Problems are reported by throwing `SofabError`; the cause is on `SofabError.code`
(`ARGUMENT`, `USAGE`, `BUFFER_FULL`, `INVALID_MSG`).
...
In streaming mode `OStream` drains its small caller buffer to a `FlushSink`
...
transport hands you and finish with `end()`. String / blob payloads arrive as one
or more chunks tagged with the field's `total` length and byte `offset`:
...
`Cursor` inverts control: your code drives the loop with one `switch` over
`cursor.id`, reading straight into typed fields, with `skip()` keeping the cursor
in sync on unknown ids. This is the shape generated message classes use:
...
`sofabgen` compiles a schema to one class per message whose `encode` /
`decodeFrom` methods call the primitives above. A nested message just recurses
into the child type's `decodeFrom`:
