      ...
      switch (this.state) {
        ...
        case S.FixlenFp: {
          ...
          if (this.fixSub === FixlenSubtype.Fp32) {
            // Pass the raw 4 LE bytes (still in `scratch`): `value` (a double)
            // would have quieted a signaling NaN (§4.6; Visitor.fp32).
            this.top().fp32?.(this.id, value, this.scratch.subarray(0, 4));
        ...
        case S.ArrayFp: {
          ...
          if (this.arrKind === ArrayKind.Fp32) {
            this.top().arrayFp32?.(this.id, this.arrIndex, value, this.scratch.subarray(0, 4));
