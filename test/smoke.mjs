// Cross-runtime smoke test for the *built* bundle (dist/).
//
// Runs identically on Node, Deno and Bun — it imports only the public ESM
// entry and uses only standard JS / Web APIs (no test framework, no `node:`
// imports), so it proves the shipped artifact stays runtime-agnostic. CI runs
// it under each runtime; `npm run build` must have produced dist/ first.
//
//   node test/smoke.mjs   |   deno run test/smoke.mjs   |   bun test/smoke.mjs

import { OStream, IStream, decode } from "../dist/index.js";
